...
// Апскейл: растяжка EASU плюс резкость RCAS — обе половины FSR 1.0.
//
...
class Multiframe {
    ...
    constructor(device: GraphicsDevice, camera: CameraComponent, samples?: Vec3[]) {
        ...
        this.easuRenderPass.events.on('execute', () => {
            ...
            // Оба прохода цепочки обязаны брать координаты одинаково: переворот в одном из них
            // выдаёт кадр вверх ногами. Проверено перебором всех четырёх сочетаний против
            // эталонной простой копии — сходятся только «оба без переворота» и «оба с ним».
            // Берём без: так же, как проход накопления, который тоже пишет в цель рендера.
            resolve(device.scope, {
                texcoordMod: [1, 1, 0, 0],
                multiframeTex: src,
        ...
        this.finalRenderPass.events.on('execute', () => {
            ...
            if (this.chained) {
                ...
                //
                // Переворот. На WebGPU проход в цель рендера переворачивает картинку, на WebGL
                // нет. Одиночный проход переворачивает при чтении цели сцены на WebGPU и не
                // переворачивает при чтении накопления. Цепочка добавляет лишнюю цель, поэтому
                // для совпадения с одиночным проходом нужно: переворот у цели сцены уже даёт
                // сама цепочка, а при накоплении его надо вернуть руками. Проверено перебором
                // сочетаний на обоих бэкендах в обоих режимах.
                resolve(device.scope, {
