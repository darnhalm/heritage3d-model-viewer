...
class Multiframe {
    ...
    constructor(device: GraphicsDevice, camera: CameraComponent, samples?: Vec3[]) {
        ...
        this.easuRenderPass.events.on('execute', () => {
            ...
            // Переворот тот же, что применил бы одиночный проход к этому источнику. Цепочка
            // обязана повторить его целиком: первый проход берёт правило источника, второй
            // выводит уже готовую картинку и ничего не меняет. Без этого кадр приходил на экран
            // вверх ногами при любом масштабе пикселя больше единицы.
            resolve(device.scope, {
