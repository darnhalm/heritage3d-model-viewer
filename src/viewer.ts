...
class Viewer {
    ...
    constructor(
        ...
        this.canvas.addEventListener('pointermove', (event: PointerEvent) => {
            // Указатель над моделью — до клика ещё есть время, и оно наше: греем пикер здесь,
            // а не на загрузке, где он отнимал бы секунды у первого кадра.
            this.warmPicker();
            const rect = this.canvas.getBoundingClientRect();
     ...
     * ровно один раз за сеанс, зато всегда.
     *
     * Делаем ту же работу, когда указатель впервые пошёл над моделью: клик ещё не случился, а
     * загрузка уже позади. На самой загрузке греть нельзя — на программном растеризаторе эта
     * отрисовка отнимает у первого кадра секунды (смоук-тест про поиск настроек ловит это
     * тридцатисекундным бюджетом).
     */
    private warmPicker() {
