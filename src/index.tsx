...
const observerData: ObserverData = {
    ...
    camera: {
        ...
        multisample: true,
        // Сила резкости RCAS: 0 выключает фильтр, 1 — полная сила алгоритма. Полная и стоит
        // по умолчанию: RCAS сам ограничивает лобу пределом 0.1875, за который не выходит,
        // поэтому «единица» здесь означает не перебор, а штатную работу фильтра.
        sharpness: 1,
        // Растяжка кадра через EASU. Выключение возвращает билинейную выборку — так можно
        // сравнить оба способа на одной сцене, пока замера качества нет.
        easu: true,
        // Штатные постэффекты CameraFrame создаются лениво. Все выключены, чтобы базовый
        // путь рендера и его стоимость оставались прежними.
        taa: false,
        ssao: false,
        ssaoIntensity: 0.5,
        ssaoRadius: 30,
