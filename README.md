...
## Спектральные слои GLB

Плеер использует стандартное расширение `KHR_materials_variants`: слой меняет материал на той же геометрии. Для автоматического размещения научных каналов на шкале добавьте к имени варианта заглавный код после `@`, например `@VIS`, `@IRR`, `@IRRFC`, `@UVL` или `@VIL`. Без `@` название считается обычным пользовательским слоем.

Распознанные каналы получают фиксированные цвета и позиции шкалы. Их цвет нельзя изменить или отключить. Для остальных вариантов цвет быстрого переключателя задаётся вручную в панели **Materials**; чёрный перечёркнутый кружок означает, что слой не показан в нижнем переключателе. На экранах шириной до 950 px нижний виджет скрывается, чтобы не перекрывать центральную панель, но слои остаются доступны в редакторе.

Материал Unlit хранится стандартно: текстура остаётся в `pbrMetallicRoughness.baseColorTexture`, а отсутствие освещения задаёт `KHR_materials_unlit`. PlayCanvas внутренне переносит карту в `emissiveMap` для отрисовки, однако редактор показывает её как **Base Color**, поскольку это назначение карты в GLB.

Встроенные изображения, используемые только альтернативными вариантами, декодируются лениво при первом выборе и затем остаются в кэше. Сам GLB скачивается целиком; оптимизация уменьшает время первого кадра и стартовую VRAM.

Демонстрационная модель: [Harpocrates multispectral](https://playcanvasviewer.website.yandexcloud.net/?webgpu&load=https%3A%2F%2Fplaycanvasviewer.website.yandexcloud.net%2Fmodels%2Fharpocrates-multispectral-tagged.glb). Источник — Kelsey Museum of Archaeology, University of Michigan; лицензия CC BY 4.0. [Полная атрибуция](https://playcanvasviewer.website.yandexcloud.net/models/harpocrates-multispectral-ATTRIBUTION.md).
