...
### Спектральный переключатель вариантов

Если имена `KHR_materials_variants` содержат поддерживаемые заглавные метки после `@`, справа внизу появляется шкала X-ray / UV / VIS / IR. Кружок переключает вариант без изменения геометрии; tooltip показывает полное имя материала. Для распознанного метода цвет фиксирован. Обычный вариант появляется рядом со шкалой после назначения цвета в **Materials**; `#000000` означает выключенное отображение. При ширине до 950 px нижний виджет скрыт, чтобы не пересекаться с центральным блоком.
