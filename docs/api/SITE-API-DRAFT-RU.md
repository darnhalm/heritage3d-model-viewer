...
## 1. Авторизация через Geoscan Login

Регистрация, пароль и базовая учетная запись находятся во внешнем Geoscan Login. Портал Heritage3D проверяет токен и ведет свои роли, организации и права.
...
## 2. Культурные объекты

Главная карточка сайта. Здесь живет H3DID, тип объекта, название, статус, организация и профиль описания.
...
Простые описательные поля объекта. Удобны для каталога, обмена, импорта и базовой совместимости.
...
## 4. CIDOC CRM metadata

Семантическая модель культурного объекта: классы CIDOC CRM, места, события, акторы, идентификаторы и связи.
...
## 5. Справочники стандартов metadata

Полные стандарты не нужно переписывать как плоскую карточку объекта. API должен отдавать справочники терминов, классов и свойств, а профиль объекта выбирает нужное под конкретный тип объекта.
...
## 6. Метаданные: отдельные поля и граф

Точечный доступ нужен, чтобы фронт или интеграция могли читать/обновлять одно поле без загрузки всего профиля. Dublin Core удобно читать как поля, CIDOC CRM дополнительно как граф nodes/relations.
...
## 7. Внешние идентификаторы

Связь H3DID с ЕГРОКН, Госкаталогом, КАМИС, Геоскан Cloud и локальными системами.
...
## 8. Коллекции и подколлекции

Коллекции группируют большое количество объектов внутри организации. parentCollectionId задает дерево коллекций и не дублирует collectionId.
...
## 9. Цифровые двойники / представления

H3DID — ID культурного объекта. twinId — внутренний ID конкретного цифрового представления: 3D, Geoscan, gigapixel 2D, point cloud, panorama.
...
## 10. Ассеты сайта и загрузка

Сайт хранит h3druAssetId и связь со storageAssetId. Файл физически уходит в storage API.
...
## 11. Технические данные модели

Сайт хранит и показывает агрегированную техническую информацию о цифровом двойнике и его ассетах. Источник части данных — storage/player processing, но фронт читает их через API сайта.
...
## 12. Квоты, тарифы и ошибки загрузки

Портал показывает пользователю понятные сообщения о лимитах, но источник технической ошибки quota_exceeded — storage API.
...
## 13. Публикация и модерация

Статусы draft/review/published хранятся на сайте. Storage не знает роли модератора.
...
## 14. Каталог и публичные страницы

Публичный сайт показывает только опубликованные или разрешенные пользователю объекты.
...
## 15. Теги, темы и категории

Теги нужны для удобного поиска и витрин, но не должны жить отдельно от metadata. Базовая связь: tag -> dc:subject; для типологии tag/category может ссылаться на CIDOC CRM E55 Type.
...
Для Heritage3D лучше трактовать лайки как избранное/закладки. Поддерживаем три уровня: организация, коллекция, объект.
...
Сайт проверяет права пользователя, затем вызывает storage/player API от имени доверенного портала.
...
## 18. Скачивание и архив

Сайт решает, можно ли скачать или архивировать asset; storage выдает только временную ссылку или выполняет техническое действие.
...
## 19. Организации / музеи

Организация является владельцем H3DID и объектов. Пользователь приходит из Geoscan Login, а членство и роли внутри организации хранит Heritage3D.
...
## 20. Статьи и подборки моделей

Статья — отдельная редакционная страница с текстом, медиа и подборкой трехмерных моделей/цифровых двойников. Она не заменяет коллекцию: коллекция группирует объекты, статья рассказывает сюжет и вставляет выбранные объекты в нужном порядке.
...
Рабочая группа — команда внутри организации. Она может работать с объектами, коллекциями и цифровыми двойниками, но владельцем H3DID остается организация.
...
## 22. Webhooks от storage

Эти endpoints находятся на сайте, но вызывает их сервер хранения.
...
Импорт и сверка данных с музейными и государственными системами.
