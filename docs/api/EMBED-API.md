...
Viewer отправляет состояние верхнего плеера POI при Play, Pause и Stop:
...
`state` принимает значения `playing`, `paused` и `stopped`. `elapsed` —
прошедшее время текущей карточки в секундах; при `stopped` оно равно `0`.
